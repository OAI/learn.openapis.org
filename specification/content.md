...
This field can be found both in [Response Objects](https://spec.openapis.org/oas/latest#response-object) and [Request Body Objects](https://spec.openapis.org/oas/latest#request-body-object). It is a map pairing standard [RFC6838 Media Types](https://tools.ietf.org/html/rfc6838) with OpenAPI [Media Type Objects](https://spec.openapis.org/oas/latest#media-type-object).
...
The [Media Type Object](https://spec.openapis.org/oas/latest#media-type-object) describes the structure of the content and provides examples for documentation and mocking purposes (examples are dealt with in the [Documentation page](docs)).
...
The [Schema Object](https://spec.openapis.org/oas/latest#schema-object) defines a data type which can be a primitive (integer, string, ...), an array or an object depending on its `type` field.
...
Array types should have an `items` field, which is a [Schema Object](https://spec.openapis.org/oas/latest#schema-object) itself, and defines the type for each element of the array. Additionally, the size of the array can be limited with `minItems` and `maxItems`.
...
Finally, object types should have a `properties` field listing the properties of the object. This field is a map pairing property names with a [Schema Object](https://spec.openapis.org/oas/latest#schema-object) defining their type. This allows building data types as complex as required.
...
- The `content` field maps Media Types to [Media Type Objects](https://spec.openapis.org/oas/latest#media-type-object).
- Each [Media Type Object](https://spec.openapis.org/oas/latest#media-type-object) has a `schema` field describing a [Schema Object](https://spec.openapis.org/oas/latest#schema-object).
- [Schema Objects](https://spec.openapis.org/oas/latest#schema-object) define a data `type` which can be customized through multiple properties like `minimum`, `maximum`, `items`, `properties` and many more.
